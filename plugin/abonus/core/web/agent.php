...
class Agent_EweiShopV2Page extends PluginWebPage
{
	...
	public function check()
	{
		...
		$members = pdo_fetchall('SELECT id,openid,nickname,realname,mobile,aagentstatus,aagentprovinces,aagentcitys,aagentareas FROM ' . tablename('ewei_shop_member') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		$time = time();
		...
		foreach ($members as $member ) {
			...
			$aagentprovinces = iunserializer($member['aagentprovinces']);
			$aagentcitys = iunserializer($member['aagentcitys']);
			$aagentareas = iunserializer($member['aagentareas']);
			$aagentprovinces = ((!(empty($aagentprovinces)) ? $aagentprovinces : array()));
			$aagentcitys = ((!(empty($aagentcitys)) ? $aagentcitys : array()));
			$aagentareas = ((!(empty($aagentareas)) ? $aagentareas : array()));
			...
			if ($status == 1) {
				...
				$this->model->sendMessage($member['openid'], array('nickname' => $member['nickname'], 'aagenttime' => $time, 'aagentareas' => implode('; ', $areas)), TM_ABONUS_BECOME);
			}
			 else {
