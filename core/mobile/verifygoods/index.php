...
class Index_EweiShopV2Page extends MobilePage 
{
	...
	public function detail() 
	{
		...
		//diy start
		if($_W['uniacid']==10){
			$tmp1 = pdo_fetchall('select verifynum  from ' . tablename('ewei_shop_verifygoods_log') . '    where verifygoodsid =:id  ', array(':id' => $item['id']));
			$tmp_num = 0;
			foreach ($tmp1 as $tmp2 ) 
			{
				$tmp_num += intval($tmp2['verifynum']);
			}
			$count_buy = pdo_fetch('select total from ' . tablename('ewei_shop_order_goods') . ' where orderid=:orderid and uniacid=:uniacid limit 1', array(':orderid' => $item['orderid'], ':uniacid' => $_W['uniacid']))['total'];
			$last_ci= intval($count_buy) - $tmp_num;
			if($last_ci<1){
				$this->message(array('title' => '剩余次数不足', 'message' => '剩余次数不足'), 'javascript:WeixinJSBridge.call("closeWindow");', 'error');
				exit();
			}
